...
OpenSpecialFile(SpecialPath aPath, FileMode aMode, FILE** aOutFile)
{
  ...
  // Get path for each computer, user specific and roaming data.
  HRESULT hr = SHGetFolderPath(NULL, CSIDL_APPDATA, NULL, 0, szPath);
  // Note: Can't call ENSURE_SUCCESS, since it would be re-entrant here!
  if (FAILED(hr)) return hr;

  // Append product-specific path.
  if (!PathAppendW(szPath, L"\\Movie Rotator")) {
    return E_FAIL;
  }

  // Ensure directory exists.
  if (_wmkdir(szPath) == -1 && errno != EEXIST) {
    return E_FAIL;
  }

  const wchar_t* filename = nullptr;
  switch (aPath) {
    case RegistrationKeyPath:
      filename = L"\\license-key.txt";
      break;
    case LogFilePath:
      filename = L"\\log.txt";
      break;
  ...
  if (!PathAppendW(szPath, filename)) {
    return E_FAIL;
  }
  ...
  // Open the file.
  FILE* file = nullptr;
  errno_t err = _wfopen_s(aOutFile, szPath, mode);
  if (err != 0) {
    return E_FAIL;
  }
...
void DBGMSG(PCWSTR format, ...)
{
  ...
  // Write to log file.
  FILE* file = nullptr;
  if (SUCCEEDED(OpenSpecialFile(LogFilePath, Append, &file))) {  
    fputws(msg, file);
    fclose(file);
  }
}
...
{
  ENSURE_TRUE(aReader, E_POINTER);
  ENSURE_TRUE(aOutFirstAudioIndex, E_POINTER);
  ENSURE_TRUE(aOutFirstVideoIndex, E_POINTER);

  *aOutFirstAudioIndex = -1;
  *aOutFirstVideoIndex = -1;
  HRESULT hr = S_OK;
  DWORD streamIndex = 0;
  while (true) {
    IMFMediaTypePtr mediaType;
    hr = aReader->GetCurrentMediaType(streamIndex, &mediaType);
    if (hr != S_OK) {
      break;
    }
    if (IsVideoType(mediaType) &&
        *aOutFirstVideoIndex == -1) {
      *aOutFirstVideoIndex = streamIndex;
    } else if (IsAudioType(mediaType) &&
               *aOutFirstAudioIndex == -1) {
      *aOutFirstAudioIndex  = streamIndex;
    } else {
      // This isn't the first audio or first video stream, disable the
      // stream so we don't waste resources decoding/demuxing it.
      hr = aReader->SetStreamSelection(streamIndex, FALSE);
    ...
    if (!bPanScan || hr == MF_E_ATTRIBUTENOTFOUND)
    {
        ...
        if (hr == MF_E_ATTRIBUTENOTFOUND)
        {
            hr = MFGetAttributeSize(
                pType, MF_MT_FRAME_SIZE, &width, &height
                );
...
{
  ENSURE_TRUE(aType != nullptr, E_POINTER);
  ENSURE_TRUE(aOutStride != nullptr, E_POINTER);

  // Try to get the default stride from the media type.
  HRESULT hr = aType->GetUINT32(MF_MT_DEFAULT_STRIDE, (UINT32*)aOutStride);
  if (SUCCEEDED(hr)) {
    return S_OK;
  }
  ...
  if (subtype == MFVideoFormat_RGB32) {
    *aOutStride = 4 * width;
    ...
    hr = MFGetStrideForBitmapInfoHeader(subtype.Data1, width, aOutStride);
    ENSURE_TRUE(SUCCEEDED(hr), hr);
  }
                        ...
                        LONGLONG* aOutDuration)
{
  ...
  hr = PropVariantToInt64(var, aOutDuration);
  ENSURE_SUCCESS(hr, hr);
...
{
  ENSURE_TRUE(aSample != nullptr, E_POINTER);
  ENSURE_TRUE(aClone != nullptr, E_POINTER);

  IMFSamplePtr clone;
  HRESULT hr;

  hr = MFCreateSample(&clone);
  ENSURE_SUCCESS(hr, hr);

  LONGLONG duration;
  hr = aSample->GetSampleDuration(&duration);
  ENSURE_SUCCESS(hr, hr);
  hr = clone->SetSampleDuration(duration);
  ENSURE_SUCCESS(hr, hr);

  DWORD flags;
  hr = aSample->GetSampleFlags(&flags);
  ENSURE_SUCCESS(hr, hr);
  hr = clone->SetSampleFlags(flags);
  ENSURE_SUCCESS(hr, hr);

  LONGLONG sampleTime;
  hr = aSample->GetSampleTime(&sampleTime);
  ENSURE_SUCCESS(hr, hr);
  hr = clone->SetSampleTime(sampleTime);
  ENSURE_SUCCESS(hr, hr);
...
typedef ULONGLONG  (WINAPI*GetTickCount64Ptr)();

uint64_t
GetTickCount64_DLL()
{
  GetTickCount64Ptr fnptr =
    (GetTickCount64Ptr)GetProcAddress(GetModuleHandle(L"kernel32.dll"),
                                                      "GetTickCount64");
  ENSURE_TRUE(fnptr, 0);
  return fnptr();
}